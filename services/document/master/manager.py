...
class SharePoint:
    ...
    def _get_folder_name(self, itens: Dict = None) -> str:
        ...
        Args:
            itens (Dict): Dicionário contendo os itens configurados
            
        Returns:
            str: Nome da pasta do SharePoint a ser utilizada
        """
        try:
            from pages.pagamento_entrega.components import ComponentsPagamentoEntrega
            ...
            if produtos_configurados.get('mt'):
            ...
            # Em caso de erro, retorna a pasta BT como fallback
            return os.getenv("SHAREPOINT_FOLDER_NAME_BT")
...
def inserir_desvios(doc):
    ...
    texto_referencia = "A não contestação das condições desta proposta às tornarão parte integrante do Pedido de Compra."
...
class DocumentManager:
    ...
    def get_template_file(itens: Dict = None):
        ...
        try:
            ...
            if produtos_configurados['mt']:
                template_name = os.getenv('TEMPLATE_MT')
                local_template_path = f"/tmp/{template_name}"
                logging.info(f"Procurando template MT: {template_name}")
            else:
                template_name = os.getenv('TEMPLATE_BT')
                local_template_path = f"/tmp/{template_name}"
                logging.info(f"Procurando template BT: {template_name}")
    ...
    def gerar_documentos(itens: Dict, observacao: str, replacements: Dict):
        """
        Gera documentos baseado no tipo de produto (MT ou BT)
        """
        try:
            # 1. Identificar tipo de produto
            produtos_configurados = ComponentsPagamentoEntrega.carregar_tipo_produto(itens)
            is_mt = produtos_configurados.get('mt', False)
            logging.info(f"Tipo de produto identificado: {'MT' if is_mt else 'BT'}")
            ...
            output_path = os.path.join(
                output_dir,
                f"proposta_{'mt' if is_mt else 'bt'}.docx"
            )
            ...
            try:
                ...
                # 6. Aplicar substituições de texto
                logging.info("Iniciando substituição de texto...")
                # word_service.substituir_texto_documento(doc, replacements)
                logging.info("Substituição de texto concluída")
                
                # 7. Inserir tabelas
                items_key = 'itens_configurados_mt' if is_mt else 'itens_configurados_bt'
                ...
                logging.info("Inserção de tabelas concluída")
                
                # 8. Processar imagens e títulos
                logging.info("Iniciando processamento de imagens...")
                if is_mt:
                    # Processamento específico para MT (sem verificação de IP)
                    logging.info("Processando imagens para MT...")
                    if hasattr(pdf_service_mt, 'inserir_titulo_e_imagem'):
                        pdf_service_mt.inserir_titulo_e_imagem(doc, itens_configurados)
                else:
                    # Processamento específico para BT (com verificação de IP)
                    logging.info("Processando imagens para BT...")
