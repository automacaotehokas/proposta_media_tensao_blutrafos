...
from backup.Inicial import carregar_cidades
...
if 'lucro' not in st.session_state:
    dadosimpostos['lucro'] = 5.0
if 'icms' not in st.session_state:
    dadosimpostos['icms'] = 12.0
if 'frete' not in st.session_state:
    dadosimpostos['frete'] = 5.0
if 'comissao' not in st.session_state:
    dadosimpostos['comissao'] = 5.0

if 'local_frete_itens' not in st.session_state:
    dadosimpostos['local_frete_itens'] = st.session_state['dados_iniciais'].get('local_frete', '')

lucro = st.number_input('Lucro (%):', min_value=0.0, max_value=100.0, step=0.1, value=dadosimpostos['lucro'])
dadosimpostos['lucro'] = lucro
icms = st.number_input('ICMS (%):', min_value=0.0, max_value=100.0, step=0.1, value=dadosimpostos['icms'])
dadosimpostos['icms'] = icms
frete = st.number_input('Frete (%):', min_value=0.0, step=0.1, value= dadosimpostos['frete'])
dadosimpostos['frete'] = frete
...
local_frete_itens = st.selectbox(
            ...
            index=st.session_state['cidades'].index(st.session_state['impostos']['local_frete_itens']) if st.session_state['impostos']['local_frete_itens'] in st.session_state['cidades'] else 0
        )
st.session_state['impostos']['local_frete'] = local_frete_itens  

contribuinte_icms = st.radio(
    "O cliente é contribuinte do ICMS?",
    options=["Sim", "Não"],
    index=0 if 'contribuinte_icms' not in dadosimpostos else 1 if dadosimpostos['contribuinte_icms'] == "Não" else 0
)
dadosimpostos['contribuinte_icms'] = contribuinte_icms

if 'difal' not in st.session_state:
    dadosimpostos['difal'] = 0.0
if 'f_pobreza' not in st.session_state:
    dadosimpostos['f_pobreza'] = 0.0

if contribuinte_icms == "Sim":
    dadosimpostos['difal'] = 0.0
    dadosimpostos['f_pobreza'] = 0.0
else:
    dadosimpostos['difal'] = st.number_input(
        'DIFAL (%):',
        min_value=0.0,
        value=dadosimpostos['difal'],
        step=0.1
    )
    dadosimpostos['f_pobreza'] = st.number_input(
        'F. Pobreza (%):',
        min_value=0.0,
        value=dadosimpostos['f_pobreza'],
        step=0.1
    )

comissao = st.number_input('Comissão (%):', min_value=0.0, step=0.1, value=dadosimpostos['comissao'])
dadosimpostos['comissao'] = comissao
...
st.session_state['configuracoes_itens']= {}
st.session_state['configuracoes_itens']['quantidade_itens']= quantidade_itens
...
# Atualizar o preço total considerando o adicional IP e demais fatores
    ...
    preco_unitario = int(((preco_base1 + adicional_ip + adicional_k + adicional_caixa_classe) * (1 - 0.12)) / \
                      (1 - (dadosimpostos['difal'] / 100) - (dadosimpostos['f_pobreza'] / 100) - (dadosimpostos['icms'] / 100)))
