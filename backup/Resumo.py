...
from backup.sharepoint_code import SharePoint  # Certifique-se de ter este módulo
from backup.replace import inserir_tabelas_word  # Certifique-se de ter esta função
    ...
    try:
        def converter_tipos(obj):
            ...
            if isinstance(obj, np.integer):
                ...
                return {key: converter_tipos(value) for key, value in obj.items()}
            elif isinstance(obj, list):
                return [converter_tipos(item) for item in obj]
            return obj

        # Dados básicos da revisão
        dados_para_salvar = {
        ...
        dados_para_salvar = converter_tipos(dados_para_salvar)
        valor_total = sum(item.get('Preço Total', 0) for item in dados_para_salvar['itens_configurados'])
        numero_revisao = int(st.session_state.dados_iniciais.get('rev', '00'))
        ...
        try:
            # Verificar se já existe uma revisão com mesmo número para esta proposta
            cur.execute("""
                SELECT id_revisao 
                FROM revisoes 
                WHERE id_proposta_id = %s AND revisao = %s
            """, (
                st.session_state.dados_iniciais.get('id_proposta'),
                numero_revisao
            ))
            
            existing_revision = cur.fetchone()
            revisao_id = st.query_params.get("revisao_id")

            # Se encontrou uma revisão existente, usar seu ID para atualização
            if existing_revision:
                revisao_id = existing_revision[0]

            # Dados dos arquivos
            word_filename = f"proposta_{st.session_state.dados_iniciais['bt']}_rev{numero_revisao}.docx"
            pdf_filename = f"proposta_{st.session_state.dados_iniciais['bt']}_rev{numero_revisao}.pdf"
            ...
            os.makedirs(os.path.dirname(full_word_path), exist_ok=True)

            # Salvar arquivos fisicamente
            buffer_word = st.session_state.get('buffer_word')
            buffer_pdf = st.session_state.get('buffer_pdf')
            ...
            if revisao_id:
                ...
                sql = """
                    ...
                    SET conteudo = %s, 
                        ...
                        revisao = %s,
                        dt_revisao = NOW(),
                        comentario = 'Revisão Inicial',
                        arquivo = %s,
                        arquivo_pdf = %s
                    WHERE id_revisao = %s::uuid
                    RETURNING id_revisao;
                """
                cur.execute(sql, (
                    json.dumps(dados_para_salvar),
                    valor_total,
                    numero_revisao,
                    word_path,
                    pdf_path,
            ...
            if resultado:
                ...
                # Após salvar e gerar documentos, adicionar script para atualizar página do Django
                if revisao_id:
                    st.markdown("""
                        <script>
...
for key in ['local_frete', 'icms', 'contribuinte_icms', 'lucro', 'frete', 'local_frete_itens', 'difal', 'f_pobreza', 'comissao']:
    if key not in dadosimpostos:
        dadosimpostos[key] = 0.0 if 'f' in key or 'c' in key else ''  # Ajuste para valores padrão
...
def gerar_documento_word():
    ...
    replacements = {
        ...
        '{{LOCALFRETE}}': str(dadosimpostos['local_frete_itens']),
        '{{ICMS}}': str(dadosimpostos['icms']).replace('.', ',') + "%",
        '{{IP}}': ', '.join(set(
...
def gerar_pdf():
    ...
    try:
        ...
        elementos_variaveis = [
            ...
            Paragraph(f"<b>Contribuinte:</b> {dadosimpostos['contribuinte_icms']}", styles['Normal']),
            Paragraph(f"<b>Lucro:</b> {dadosimpostos['lucro']:.2f}%", styles['Normal']),
            Paragraph(f"<b>ICMS:</b> {dadosimpostos['icms']:.2f}%", styles['Normal']),
            Paragraph(f"<b>Frete:</b> {dadosimpostos['frete']:.2f}%", styles['Normal']),
            Paragraph(f"<b>Comissão:</b> {dadosimpostos['comissao']:.2f}%", styles['Normal']),
            Paragraph(f"<b>DIFAL:</b> {dadosimpostos['difal']:.2f}%", styles['Normal']),
            Paragraph(f"<b>F.pobreza:</b> {dadosimpostos['f_pobreza']:.2f}%", styles['Normal']),
            Paragraph(f"<b>Local Frete:</b> {dadosimpostos['local_frete_itens']}", styles['Normal']),
        ]
...
def pagina_gerar_documento():
    ...
    st.write(f"**Contribuinte:** {dadosimpostos['contribuinte_icms']}")
    st.write(f"**Lucro:** {dadosimpostos['lucro']:.2f}%")
    st.write(f"**ICMS:** {dadosimpostos['icms']:.2f}%")
    st.write(f"**Frete:** {dadosimpostos['frete']:.2f}%")
    st.write(f"**Comissão:** {dadosimpostos['comissao']:.2f}%")
    st.write(f"**DIFAL:** {dadosimpostos['difal']:.2f}%")
    st.write(f"**F.pobreza:** {dadosimpostos['f_pobreza']:.2f}%")
    st.write(f"**Local Frete:** {dadosimpostos['local_frete_itens']}")
    ...
    # Adicionar botão de salvar no seu layout

    if 'salvar_clicked' not in st.session_state:
        st.session_state.salvar_clicked = False

    # Botão para Confirmar e gerar documentos
    if st.button('Confirmar', disabled=not dados_completos, key='btn_salvar'):
        st.session_state.salvar_clicked = True
        if dados_completos:
            ...
            if buffer_word and buffer_pdf:
                # Primeiro salva a revisão no banco
                if validar_dados():
                    try:
                        salvar_revisao()
                        ...
                        st.error(f"Erro ao salvar revisão: {str(e)}")
                        st.stop()  # Para a execução se houver erro no salvamento
                ...
                # Após salvar e gerar documentos, adicionar script para atualizar página do Django
                if st.query_params.get("revisao_id"):
                    st.markdown("""
                        <script>
                            window.opener.location.reload();
                        </script>
